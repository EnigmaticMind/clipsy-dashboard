...
- 🔐 **OAuth Authentication** - Secure authentication with Etsy using PKCE flow
- 📥 **Download Listings** - Export all your Etsy listings to CSV format
- 📤 **Upload with Preview** - Upload edited CSV files and preview changes before applying
- ✏️ **Bulk Editing** - Edit listings locally in Excel or any spreadsheet editor
- 🎯 **Selective Updates** - Choose which changes to apply from the preview
- 🔄 **Variation Support** - Full support for listings with variations
- 🗑️ **Delete Support** - Delete listings or variations by setting SKU to "DELETE"
...
│   │   └── AuthPage.tsx      # OAuth redirect handler
│   ├── components/            # React components
│   │   ├── UploadPreview.tsx # Preview modal for changes
│   │   ├── LoadingSpinner.tsx # Loading indicator
│   │   └── EtsyTrademark.tsx # Etsy trademark notice
│   └── services/              # Business logic services
│       ├── oauth.ts          # OAuth authentication
│       ├── etsyApi.ts        # Etsy API client
│       ├── csvService.ts     # CSV generation
│       ├── uploadService.ts  # CSV parsing
│       ├── previewService.ts # Change detection & preview
│       ├── applyService.ts   # Apply changes to Etsy
│       └── listingOperations.ts # Listing CRUD operations
